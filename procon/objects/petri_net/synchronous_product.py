...
from procon.objects.petri_net.utils import add_arc_from_to, remove_arc, is_reset_arc, is_inhibitor_arc, is_petri_net
    ...
    for t1 in pn1.transitions:
        for t2 in pn2.transitions:
            if t1.label == t2.label:
                ...
                for a in t1.in_arcs:
                    add_arc_from_to(p1_map[a.source], sync, sync_net, properties=a.properties)
                for a in t2.in_arcs:
                    add_arc_from_to(p2_map[a.source], sync, sync_net, properties=a.properties)
                for a in t1.out_arcs:
                    add_arc_from_to(sync, p1_map[a.target], sync_net, properties=a.properties)
                for a in t2.out_arcs:
                    add_arc_from_to(sync, p2_map[a.target], sync_net, properties=a.properties)
...
def __copy_into(source_net, target_net, upper, skip):
    ...
    for t in source_net.transitions:
        for a in t.in_arcs:
            add_arc_from_to(p_map[a.source], t_map[t], target_net, properties=a.properties)
        for a in t.out_arcs:
            add_arc_from_to(t_map[t], p_map[a.target], target_net, properties=a.properties)

    return t_map, p_map


def construct_estimation_net(sync_net, cost_func):
    estimation_net = deepcopy(sync_net)
    estimation_net.name = 'Estimation net of ' + sync_net.name

    reset_places = [place for place in estimation_net.places if len([arc for arc in place.out_arcs if is_reset_arc(arc)]) > 0]
    for place in reset_places:
        t = PetriNet.Transition("t-reset-" + str(place.name), label=None)
        estimation_net.transitions.add(t)
        cost_func[t] = 0
        add_arc_from_to(place, t, estimation_net)

    reset_arcs = [arc for arc in estimation_net.arcs if is_reset_arc(arc)]
    for arc in reset_arcs:
        remove_arc(estimation_net, arc)

    inhibitor_arcs = [arc for arc in estimation_net.arcs if is_inhibitor_arc(arc)]
    for arc in inhibitor_arcs:
        remove_arc(estimation_net, arc)

    return estimation_net, cost_func
